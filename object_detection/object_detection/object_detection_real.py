...
class ObjectDetectionReal(Node):
    def __init__(self) -> None:
        super().__init__('object_detection_real_node')
        self.get_logger().info(f"Object Detetion for real bots")
    ...
    def depth_sensor_callback(self, msg: PointCloud2) -> None:

        try:
            # Convert PointCloud2 msg to pcl point cloud
            # New frame: base_link (not camera frame anymore)
            cloud = self.from_ros_msg(msg)

            # Filtered cloud for surface detection
            # distances from base_link
            filtered_cloud_plane = self.filter_cloud(cloud, max_x_dist=1.5, min_height=-0.35, max_height=0.4)
            ...
            bench_clusters, surface_centroids, surface_dimensions = self.extract_clusters(plane_cloud, "Bench")
...
def main(args=None) -> None:
    ...
    object_detection = ObjectDetectionReal()
