...
public:
  ...
  ~PickAndPlaceTrajectory() {
    RCLCPP_INFO(LOGGER, "Class Terminated: Pick And Place Real Trajectory");
  }

  void execute_trajectory_plan();
  void print_current_joint_values();
...
PickAndPlaceTrajectory::PickAndPlaceTrajectory(
  ...
  // Default position
  object_position_.x = 0.343;
  object_position_.y = 0.132;
...
void PickAndPlaceTrajectory::execute_trajectory_plan() {
  float pick_height = 0.19;
  float retreat_height = 0.65;
  float pregrasp_height = 0.35; // pick_height <= pregrasp <= retreat_height
  ...
  while (!object_detected_ && attempt <= 3) {
    ...
    RCLCPP_WARN(LOGGER, "Waiting for object detection...");
  }
  if (!object_detected_) {
    RCLCPP_ERROR(LOGGER, "Object detection failed. Exiting...");
    exit(-1);
  }
  // Add constraints to the pregrasp pose
  moveit_msgs::msg::Constraints constraints;
  constraints.name = "pregrasp_constraints";

  // Constrain the shoulder lift joint
  moveit_msgs::msg::JointConstraint shoulder_lift_constraint;
  shoulder_lift_constraint.joint_name = "shoulder_lift_joint";
  // Midpoint of the range in radians
  shoulder_lift_constraint.position = -120.0 * M_PI / 180.0;
  // Upper tolerance (50deg)
  shoulder_lift_constraint.tolerance_above = 50.0 * M_PI / 180.0;
  // Lower tolerance (50deg)
  shoulder_lift_constraint.tolerance_below = 50.0 * M_PI / 180.0;
  shoulder_lift_constraint.weight = 1.0; // Weight of the constraint
  constraints.joint_constraints.push_back(shoulder_lift_constraint);

  // Set the constraints for the move group
  move_group_robot_->setPathConstraints(constraints);

  RCLCPP_INFO(LOGGER, "Going to Pregrasp Position: [X: %f, Y: %f, Z: %f]",
              object_position_.x, object_position_.y, pregrasp_height);
  setup_goal_pose_target(object_position_.x, object_position_.y,
                         pregrasp_height, -1.0, 0.0, 0.0, 0.0);
  plan_trajectory_kinematics();
  execute_trajectory_kinematics();
  ...
  setup_waypoints_target(0.0, 0.0, -(pregrasp_height - pick_height));
  plan_trajectory_cartesian();
  execute_trajectory_cartesian();

  // Clear constraints after planning (optional)
  ...
  setup_waypoints_target(0.0, 0.0, +(retreat_height - pick_height));
  plan_trajectory_cartesian();
  execute_trajectory_cartesian();

  RCLCPP_INFO(LOGGER, "Rotate 180 to Release Position...");
  current_state_robot_ = move_group_robot_->getCurrentState(10);
  current_state_robot_->copyJointGroupPositions(joint_model_group_robot_,
                                                joint_group_positions_robot_);
  ...
  RCLCPP_INFO(LOGGER, "Returning to Default Position...");
  setup_joint_value_target(0.0, -1.57, 0.0, -1.57, 0.0, 0.0);
  plan_trajectory_kinematics();
  execute_trajectory_kinematics();
...
void PickAndPlaceTrajectory::object_detection_callback(
  ...
  object_position_.x = round(msg->position.x * 100.0) / 100.0 + error_x;
  object_position_.y = round(msg->position.y * 100.0) / 100.0 + error_y;
  RCLCPP_DEBUG(LOGGER, "Object detected at: [X: %f, Y: %f, Z: %f]",
               object_position_.x, object_position_.y, msg->position.z);
  object_detected_ = true;
}

void PickAndPlaceTrajectory::print_current_joint_values() {
  // Get the current state of the robot
  current_state_robot_ = move_group_robot_->getCurrentState(10);
  // Copy the current joint positions to the joint_group_positions_robot_
  // vector
  current_state_robot_->copyJointGroupPositions(joint_model_group_robot_,
                                                joint_group_positions_robot_);
...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<rclcpp::Node> base_node =
      std::make_shared<rclcpp::Node>("pick_and_place_real_perception");
