...
void PickAndPlaceTrajectory::execute_trajectory_plan() {
  ...
  RCLCPP_INFO(LOGGER, "Going to Pregrasp Position: [X: %f, Y: %f]",
              object_position_.x, object_position_.y);
  setup_goal_pose_target(object_position_.x, object_position_.y, 0.34, -1.0,
                         0.0, 0.0, 0.0);
  plan_trajectory_kinematics();
  execute_trajectory_kinematics();
